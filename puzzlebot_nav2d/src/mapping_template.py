...
class Mapper:
    def __init__(self, map_width = 100, map_height = 80, map_resolution=1.0):
        """
        Arguments
        ---------
        map_width : float
        ...
        self.map2d = np.full((map_width, map_height),-1,dtype=np.int8) #Change the original array with a 2D array of -1
        self.scan = None
        self.odom = None
    ...
    def mapit(self):
		...
		#    rows and pixels of the map (occupancy grid).
                ...
                for xy_ in xy_m:
                    #print(xy_)
                    ray_to_pixels(self.scan,orig_m[0], orig_m[1], xy_[0], xy_[1], self.map.info.resolution, self.map2d)
...
def ray_to_pixels(scan,xr, yr, x, y, map_resolution, map):
    ...
    for i in range(int(n_pixels)):
        ...
        """
        We increment in X and Y axis depending in how many meters per pixel we increment each time
        we add one pixel of the orignal laser.
        """
        x_ = xp + int(i*v[0]/map_resolution)
        y_ = yp + int(i*v[1]/map_resolution)
        #If the point in the map is already 100 we cant cahange it anymore
        if map[y_, x_] != 100:
            if dis < (max_range-0.1):
                map[y_, x_] = 0
    # Set last pixel of ray to 100
    #If the distance of the laser is less than one meter we can change the pixel to one humndred
    #We do this because localization that are so far away can be wrong
    if dis <= 1:
...
def scan_to_map_coordinates(scan, odom, origin):
    ...
    # scan_endpoints =
    scan_endpoints = []
    #We create a map from the lowest value to the highest value of tha LIDAR
    angles = np.linspace(scan.angle_min,scan.angle_max,720)
    #For each laser we get the position in robot frame
    for i in range(len(scan.ranges)):
        #We transform the large ang angle on the laser to robot frame.
        x,y = polar_to_cartesian(scan.ranges[i],angles[i])
        #We transform the position of the laser in the robot frame to the map frame
        p = np.array([[x],[y],[1]])
        p = np.dot(TT,p)
        #We add the point in map frame into a list
        scan_endpoints.append((p[0,0],p[1,0]))
    #-----------------------------------------------------
...
def polar_to_cartesian(r, th):
    ...
    #--------------------------------------------------------------
    # Your code here
    #We get the position of the laser usigo trigonometric functions
    x = r*np.cos(th)
    y = r*np.sin(th)
    return (x,y)
    #--------------------------------------------------------------
